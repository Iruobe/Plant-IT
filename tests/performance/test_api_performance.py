...
class TestResponseTimes:
    ...
    def test_list_plants_response_time(
        self, time_request, response_time_threshold, create_test_plant
    ):
        """
        ...
        assert (
            avg_duration < response_time_threshold["list_plants"]
        ), f"Average response time {avg_duration:.1f}ms exceeds threshold"

    def test_get_plant_response_time(
        self, time_request, response_time_threshold, create_test_plant
    ):
        """
        ...
        for _ in range(10):
            response, duration = time_request(
                "GET", f"/api/v1/plants/{plant['plant_id']}"
            )
            assert response.status_code == 200
            durations.append(duration)
    ...
    def test_today_tasks_response_time(
        self, time_request, response_time_threshold, create_test_care_task
    ):
        """
    ...
    def test_complete_task_response_time(
        self, time_request, response_time_threshold, create_test_care_task
    ):
        """
        ...
        for task in tasks:
            response, duration = time_request(
                "POST", f"/api/v1/care-plans/complete/{task['task_id']}"
            )
            assert response.status_code == 200
            durations.append(duration)
...
class TestConcurrentRequests:
    ...
    def test_concurrent_task_completions(
