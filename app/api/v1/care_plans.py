...
"""
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate care plan: {str(e)}"
        )
...
async def complete_task(
    ...
    completion = care_plans_repo.mark_task_complete(
        user_id=user_id, task_id=task_id, completed_date=body.completed_date
    )
...
async def uncomplete_task(
    ...
    care_plans_repo.unmark_task_complete(
        user_id=user_id, task_id=task_id, completed_date=completed_date
    )
